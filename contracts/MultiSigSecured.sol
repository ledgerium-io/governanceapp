...
contract MultiSigSecured {
    ...
    */
	...
	function voteFor(string memory _method, address _sender) internal returns(bool) {
		string memory message = append("Ballot should exist with ", _method);
		require(isBallotActive(_method), message);
		
		require(!hasVoted(_method, _sender), "already voted by the candidate on the same method");
		uint32 votesFor = ballotMap[_method].votesFor.add(1);
	...
	function voteAgainst(string memory _method, address _sender) internal returns(bool) {
		...
		require(!hasVoted(_method, _sender), "already voted by the candidate on the same method");
		uint32 votesAgainst = ballotMap[_method].votesAgainst.add(1);
	...
	function createBallot(string memory _method, address _newAddress, bool _initialVote, uint32 _minVotes, address _sender) internal returns(bool) {
		string memory message = append("Ballot exists with ", _method);
		require(!isBallotActive(_method), message);
		...
		if (_initialVote)
			votesFor = votesFor.add(1);
		else
			votesAgainst = votesAgainst.add(1);
		address[] memory temp;
